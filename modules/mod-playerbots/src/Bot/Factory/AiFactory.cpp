...
void AiFactory::AddDefaultNonCombatStrategies(Player* player, PlayerbotAI* const facade, Engine* nonCombatEngine)
{
    ...
    // CONQUEST PATCH — la stratégie "travel" doit être active par défaut sur tous
    // les bots Conquest pour que MoveToTravelTargetAction puisse pathfinder vers
    // les bannières assignées au dispatch. Sans ça, ResetStrategies (sur teleport,
    // master update) wipe le +travel et le bot abandonne sa cible.
    if (sConfigMgr->GetOption<bool>("AiPlayerbot.ConquestZoneSpawn", false))
    {
        nonCombatEngine->addStrategy("travel", false);
        // grind + move random : bots hostiles ET mobiles même au repos (sur banner
        // après arrivée). "grind" déclenche "attack anything" → bot scanne et engage
        // tout ennemi à portée. "move random" garde le bot mobile.
        nonCombatEngine->addStrategy("grind", false);
        nonCombatEngine->addStrategy("move random", false);
        nonCombatEngine->removeStrategy("rpg", false);
        nonCombatEngine->removeStrategy("new rpg", false);
    }
