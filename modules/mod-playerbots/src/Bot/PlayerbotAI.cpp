...
void PlayerbotAI::Reset(bool full)
{
    ...
    if (full)
    {
        ...
        // CONQUEST PATCH — préserve une TravelTarget de type "ConquestBannerDestination"
        // à travers les Reset(true) (teleport, login). Sans ça, le bot oublie sa cible
        // banner dès qu'il change de map → walk continu impossible.
        TravelTarget* currentTarget = aiObjectContext->GetValue<TravelTarget*>("travel target")->Get();
        bool preserveConquest = currentTarget && currentTarget->getDestination()
            && currentTarget->getDestination()->getName() == "ConquestBannerDestination";

        if (!preserveConquest)
        {
            currentTarget->setTarget(TravelMgr::instance().nullTravelDestination,
                                     TravelMgr::instance().nullWorldPosition, true);
            currentTarget->setStatus(TRAVEL_STATUS_EXPIRED);
            currentTarget->setExpireIn(1000);
        }
        else
        {
            // Re-arme forced + statut TRAVEL pour vaincre les autres actions concurrentes
            currentTarget->setForced(true);
            currentTarget->setStatus(TRAVEL_STATUS_TRAVEL);
        }
        rpgInfo = NewRpgInfo();
    }
