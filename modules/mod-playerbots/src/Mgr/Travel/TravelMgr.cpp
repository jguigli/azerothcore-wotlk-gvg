...
void TravelMgr::Init()
{
    ...
    // CONQUEST PATCH — load full TravelNode graph (3781 nodes, 15041 links) from DB
    // so MoveToTravelTargetAction peut faire du pathfinding long-distance (cross-zone).
    // Sans ça, seul le navmesh local (450y) marche, et les bots stagnent loin du target.
    sTravelNodeMap.loadNodeStore();
    LOG_INFO("playerbots", "Playerbots Taxi graph + TravelNode store loaded ({} nodes).",
